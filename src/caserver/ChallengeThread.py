...
class ChallengeThread(threading.Thread):
    def __init__(self, central_authority_server):
        ...
        self.current_challenge_command = commands.GetCurrentChallengeCommand(
    ...
    def __init_generators(self):
        self.generators.append(
            ReverseSortedListChallenge(
                self.central_authority_server.config_file))
        self.generators.append(
            SortedListChallenge(
                self.central_authority_server.config_file))
        self.generators.append(ShortestPathChallenge(
            self.central_authority_server.config_file))
    ...
    def __init_challenges(self):
        ...
        if self.current_challenge is None:
            ...
            print(
                "New challenge generated for {0} minutes".format(
                    self.current_challenge.duration))

        self.current_challenge.fill_prefix(
    ...
    def run(self):
        while self.alive:
            ...
            if timestamp > self.current_challenge.expiration():
                ...
                authority_wallet = self.database.get_wallet_by_nid(
                    self.central_authority_server.authority_wallet.nid)
                    ...
                    if submission.submitted_on < self.current_challenge.expiration():
                        ...
                        try:
                            solution = generator.generate_solution(
                                self.last_solution_hash, submission.nonce)
                        except BaseException:
                            print(
                                "Invalid submission from {0}, deleting from Database".format(
                                    submission.wallet.id))
                            self.add_invalid_submission(submission)
                            self.database.delete_submission(submission)
                            continue

                        if solution.hash.startswith(
                                self.current_challenge.hash_prefix):
                            # we got a solution
                            print(
                                "Solution valid Original Nonce:{0}  Solution Nonce:{1}".format(
                            ...
                            coin_value = decimal.Decimal(
                                self.current_challenge.coin_value)
                            if not self.central_authority_server.emit_coins:
                                # overriding coin value for 0
                                coin_value = decimal.Decimal(0)

                            recipient_wallet = self.database.get_wallet_by_nid(
                                submission.wallet.nid)

                            txn = Transaction.Transaction(
                                0, authority_wallet.id, recipient_wallet.id, coin_value)
                            txn.sign_with(ca_private_key)

                            print(
                                "{0} + {1:.5f} Coins".format(recipient_wallet.id, coin_value))
                        ...
                        print(
                            "Submission #{0} has a timestamp before the current challenge!".format(
                                submission.id))
    ...
    def add_invalid_submission(self, submission):
        invalid_submission = RequestControl.InvalidSubmission(
            submission.remote_ip, submission.wallet.nid)
        self.database.add_invalid_submission(invalid_submission)

        # fetching the invalid_submission_count
        invalid_submission_count = self.database.get_invalid_submission_count(
            submission.wallet.nid)
        if invalid_submission_count >= self.central_authority_server.invalid_submission_allowed:
            ...
            last_cooldown = self.database.get_submission_latest_cooldown(
                submission.wallet.nid)
            if last_cooldown is not None:
                cooldown_length = last_cooldown.length * 2

            submission_cooldown = RequestControl.SubmissionCooldown(
                submission.wallet.nid, cooldown_length)
            self.database.add_submission_cooldown(submission_cooldown)
            print(
                "Client {0} has been put on a cooldown for {1} minutes (Too much invalid submissions)".format(
                    submission.wallet.nid, int(
                        cooldown_length / 60)))
    ...
    def next_challenge(self):
        created_challenges = self.database.get_challenges_by_status(
            0, 1, Challenge.Created)
        new_challenge = None
        ...
        self.central_authority_server.push_message_to_miners(
            json.dumps(challenge_message))

    def generate_new_challenge(self):
        # reload server config file
        self.central_authority_server.read_vars_from_config()

        challenge_name = random.choice(
            self.central_authority_server.available_challenges)
        generator = None
        for g in self.generators:
