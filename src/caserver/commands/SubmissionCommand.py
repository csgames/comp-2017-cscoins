...
class SubmissionCommand(BaseCommand):
    ...
    def execute(self, response, client_connection, args):
        ...
        if not self.central_authority_server.is_ip_allowed_to_submit(
                remote_ip):
        ...
        try:
            ...
            submission = Submission(
                current_challenge.id, nonce, wallet, remote_ip)
            ...
            if self.database.is_wallet_disqualified(
                    current_challenge.id, wallet.nid):
